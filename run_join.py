...
def run_one(name, policy, delta, overfetch, embedder, backend, data_dir,
    ...
    ctx = pipeline.prepare_scenario(name, embedder, backend, data_dir,
                                    n_docs, vec_cache)
    joiner = VersionJoiner(ctx["store"], ctx["index"])
    ...
    # retrieval metrics on the FINAL (post-policy) hits
    triples = [(r["query"],
                [(cid, s, m) for cid, s, m, _ok, _a in r["hits"]],
...
def main():
    ...
    path = os.path.join(args.data_dir, "join_results.json")
    with open(path, "w") as f:
        json.dump(rows, f, indent=2)
    print(f"[join] wrote {path}")
    write_before_after(rows, args.data_dir)
    write_citation_table(rows, args.data_dir)
    write_delay_sla_curve(rows, args.data_dir)
...
def write_before_after(rows, data_dir):
    ...
    path = os.path.join(data_dir, "join_before_after.md")
    with open(path, "w") as f:
        f.write("\n".join(lines) + "\n")
    print(f"[join] wrote {path}")


def write_citation_table(rows, data_dir):
    ...
    path = os.path.join(data_dir, "citation_before_after.md")
    with open(path, "w") as f:
        f.write("\n".join(lines) + "\n")
    print(f"[join] wrote {path}")


def write_delay_sla_curve(rows, data_dir):
    ...
    for name in DELAY_SCENARIOS:
        for cfg in ["off", "drop@d0s", "drop@d5s", "drop@d60s"]:
            ...
            lines.append(f"| {name} | {cfg} | {t['stale_hit_rate']} | "
                         f"{t['recall_at_k']} | {t['p95_latency_ms']} |")
    path = os.path.join(data_dir, "delay_sla_curve.md")
    with open(path, "w") as f:
        f.write("\n".join(lines) + "\n")
    print(f"[join] wrote {path}")
