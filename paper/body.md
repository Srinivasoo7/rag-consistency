...
We do not claim hallucination-free RAG, temporal supersession of coexisting facts, a production incidence rate — or a new mechanism: the version join is version-stamped reads with read repair (§7). The contribution is vocabulary and a measuring stick: definitions that split retrieval staleness from citation staleness, and a harness that puts a number on how much an index lies under silent failure. The result is a lower stale-citation rate plus an explicit freshness SLO.
...
RAG’s retrieval layer is an eventually consistent cache of a source of truth. Without a join, that fact is invisible to the generator. With a version join and a lag bound the pipeline can actually meet, stale-hit and extractive stale-citation go to zero on the faults we can inject, and the bill arrives as recall and as answers about deleted entities drawn from their neighbors. That is a freshness SLO, not a claim that the model stopped hallucinating. Stated bluntly, the contribution is vocabulary and a measuring stick, not a mechanism: a two-layer staleness vocabulary that evaluation tooling can adopt, a harness that quantifies index dishonesty under the faults a worker can silently commit, and the negative result that a lag bound buys nothing the version check does not already give.
